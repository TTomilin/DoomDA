...
from sample_factory.envs.doom.action_space import doom_action_space, \
    ...
    doom_action_space_extended, doom_turn_and_attack, doom_turn_and_move_forward, \
    doom_action_space_simplified_no_weap, doom_lateral_movement
...
from sample_factory.envs.env_wrappers import ResizeWrapper, RewardScalingWrapper, TimeLimitWrapper, \
...
from sample_factory.envs.doom.action_space import doom_action_space, \
    doom_action_space_full_discretized, doom_action_space_basic, doom_action_space_discretized_no_weap, \
    doom_action_space_extended, doom_turn_and_attack
from sample_factory.envs.doom.doom_gym import VizdoomEnv
from sample_factory.envs.doom.doom_model import register_models
from sample_factory.envs.doom.wrappers.additional_input import DoomAdditionalInput
from sample_factory.envs.doom.wrappers.bot_difficulty import BotDifficultyWrapper
...
BATTLE_REWARD_SHAPING = (DoomRewardShapingWrapper, dict(reward_shaping_scheme=REWARD_SHAPING_BATTLE,
                                                        true_reward_func=None))  # "true" reward None means just the env reward (monster kills)
BOTS_REWARD_SHAPING = (
    DoomRewardShapingWrapper,
    dict(reward_shaping_scheme=REWARD_SHAPING_DEATHMATCH_V0, true_reward_func=true_reward_frags))

DEATHMATCH_REWARD_SHAPING = (DoomRewardShapingWrapper, dict(reward_shaping_scheme=REWARD_SHAPING_DEATHMATCH_V1,
...
DOOM_ENVS = [
    ...
    # GViZDoom environments
    DoomSpec('doom_defend_the_center', 'defend_the_center/defend_the_center.cfg', doom_turn_and_attack(), 1.0),
    DoomSpec('doom_health_gathering', 'health_gathering/health_gathering.cfg', doom_turn_and_move_forward(), 1.0),
    DoomSpec('doom_seek_and_kill', 'seek_and_kill/seek_and_kill.cfg', doom_action_space_simplified_no_weap(), 1.0),
    DoomSpec('doom_dodge_projectiles', 'dodge_projectiles/dodge_projectiles.cfg', doom_lateral_movement(), 1.0),
    ...
    DoomSpec('doom_defend_the_line', 'defend_the_line.cfg', doom_turn_and_attack(), 1.0),
    DoomSpec(
...
def make_doom_env_impl(
    ...
    if player_id is None:
        env = VizdoomEnv(
            doom_spec.action_space, doom_spec.env_spec_file, cfg.task, skip_frames=skip_frames, async_mode=async_mode,
        )
    else:
    ...
    if resolution is None:
        resolution = '256x144' if cfg.wide_aspect_ratio else '640x480'
...
def make_doom_env(env_name, cfg, **kwargs):
    ...
    # spec = doom_env_by_name(f'{env_name}_{cfg.task}')
