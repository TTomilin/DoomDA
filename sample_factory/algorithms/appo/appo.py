...
from sample_factory.algorithms.appo.eval import evaluate
...
class APPO(ReinforcementLearningAlgorithm):
    ...
    def add_cli_args(cls, parser):
        ...
        arg('--adam_eps', default=1e-6, type=float,
        ...
        arg('--gae_lambda', default=0.95, type=float,
        ...
        arg('--num_workers', default=multiprocessing.cpu_count(), type=int,
        ...
        arg('--rnn_type', default='gru', choices=['gru', 'lstm'], type=str,
        ...
        arg('--ppo_clip_ratio', default=0.1, type=float,
            help='We use unbiased clip(x, 1+e, 1/(1+e)) instead of clip(x, 1+e, 1-e) in the paper')
        arg('--ppo_clip_value', default=1.0, type=float,
        ...
        arg('--ppo_epochs', default=1, type=int,
        ...
        arg('--policy_workers_per_policy', default=1, type=int,
        ...
        arg('--learner_main_loop_num_cores', default=1, type=int,
        ...
        arg('--actor_worker_gpus', default=[], type=int, nargs='*',
        ...
        arg('--wandb_user', default=None, type=str,
        ...
        arg('--wandb_group', default=None, type=str,
        ...
        arg('--wandb_tags', default=[], type=str, nargs='*',
        ...
        arg('--sampler_only', default=False, type=str2bool,
            help='Do not send experience to the learner, measuring sampling throughput')

        # Evaluation
        arg('--eval_episodes', default=3, type=int, help='Number of episodes to evaluate')
        arg('--eval_frequency', default=1_000_000, type=int, help='Number of time steps after to perform evaluation')
        arg('--eval_render', default=False, type=str2bool, help='Enables the rendering of evaluation episodes')
    ...
    def __init__(self, cfg):
        ...
        self.eval_freq = cfg.eval_frequency
        self.last_eval = 0
            ...
            if 'episodic' in report:
                ...
                for _, key, value in iterate_recursively(s):
                    if key not in self.policy_avg_stats:
                        self.policy_avg_stats[key] = [deque(maxlen=self.cfg.stats_avg) for _ in
        ...
        with timing.timeit('experience'):
            ...
            try:
                while not self._should_end_training():
                    ...
                    steps = self.env_steps[eval_policy_id] if self.env_steps else 0
                    if first_evaluation or steps - self.last_eval >= self.eval_freq:
                        first_evaluation = False
                        Thread(target=evaluate, args=(self.cfg, self.writers[eval_policy_id], steps, self.cfg.eval_tasks)).start()
                        self.last_eval = steps
